...
|---:|---:|---|---|---|
| 0 | 1週目 | 目的、非目標、安全な実験範囲を固定する | Project Charter、Rules of Engagement、テストデータ方針 | 実験の価値・範囲・禁止事項を説明できる |
...
- 実験では架空の組織、データ、credentialだけを使う方針に合意している

### やること

- [ ] Project Charterに目的、対象読者、成功条件、非目標、8週間の時間上限を書く
- [ ] 「Promptによる教育」と「下位層による強制」を別々に検証する仮説を書く
- [ ] Rules of Engagementに許可対象、禁止対象、停止条件、架空データ限定を明記する
- [ ] テストデータ方針に、架空の組織・人物・文書・credential・Canaryだけを使うことを明記する
- [ ] 評価artifactと公開文書に対するredaction、保存、公開範囲の原則を決める
...
| テストデータ方針 | `docs/test-data-policy.md` | 架空データ、Canary、credential、redaction、保存・公開範囲 |
| ADRテンプレート | `docs/adr/0000-template.md` | Context、Decision、Consequences、Status |

### 完了条件

- [ ] 検証する仮説と、それを反証できる観測結果がProject Charterに書かれている
- [ ] 実在システム、公開インターネット、第三者サービスを対象外と明記している
- [ ] fixture、credential、Canaryがすべて架空であることを確認する基準がある
